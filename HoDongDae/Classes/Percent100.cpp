...
void Percent100::storyAAA(Ref *pSender)
{
	...
	char MyString[400];
	WideCharToMultiByte(CP_UTF8, 0, L"벌금 패널티를 받았다.\n\n(소지금이 감소합니다.)", -1, MyString, 400, NULL, NULL);
	auto name = Label::createWithTTF(MyString, "Percent100/ABC.ttf", 30);
	name->setAnchorPoint(Point(0, 1));
	name->setPosition(Point(40, 150));
	name->setColor(Color3B::WHITE);
	...
	Singletons::getInstance()->money = Singletons::getInstance()->money - (Singletons::getInstance()->money*0.3);
