...
void QuizScene::setLabelQuiz() // 정답 수
{
...
void QuizScene::initMenuA() // 답 번호
{
	...
	auto button_2 = MenuItemImage::create(
		...
		CC_CALLBACK_1(QuizScene::CheckA_2, this));
	...
	auto button_3 = MenuItemImage::create(
		...
		CC_CALLBACK_1(QuizScene::CheckA_3, this));
	...
	auto button_4 = MenuItemImage::create(
		...
		CC_CALLBACK_1(QuizScene::CheckA_4, this));
...
void QuizScene::CheckA_1(Ref * pSender) // 1번 답 확인
{
	if (i == 3 || i == 7 || i == 8) // 1번 정답
	{
		...
		if (Singletons::getInstance()->Qnum == 2)
		{
			...
			auto scene = TransitionCrossFade::create(1.5, QuizEndScene::createScene());
		...
		else if (Singletons::getInstance()->Qnum < 2)
		{
			...
			Director::getInstance()->getEventDispatcher()->removeAllEventListeners();
			auto scene = TransitionCrossFade::create(1.5, QuizScene::createScene());
			Director::getInstance()->replaceScene(scene);
		}
	}
	else
...
void QuizScene::CheckA_2(Ref * pSender) // 2번 답 확인
{
	if (i == 0 || i == 4) // 2번 정답
	{
		auto spr = CCSprite::create("QuizGame/o.png");
...
void QuizScene::CheckA_3(Ref * pSender) // 3번 답 확인
{
...
void QuizScene::CheckA_4(Ref * pSender) // 4번 답 확인
{
