...
void MiniGameMainScene_1::SetLabel(float delta) // 시간 변화
{
	...
	if (currClock == 0) // 시간이 다 될 경우
	{
		...
		Singletons::getInstance()->bongnum = 1;
	}
}
...
void MiniGameMainScene_1::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) // 방향키 처리
{
	...
	switch (keyCode)
	{
		...
		case EventKeyboard::KeyCode::KEY_RIGHT_ARROW:
		{
			...
			for (int i = 0; i < 3; i++) {
				ani->addSpriteFrameWithFile(StringUtils::format("ServiceMiniGame/move3.png", i + 1));
			}
			ani->addSpriteFrameWithFile("ServiceMiniGame/move1.png");
