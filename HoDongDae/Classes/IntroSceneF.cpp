...
Scene* IntroSceneF::createScene()
{
    auto scene = Scene::create();
    auto layer = IntroSceneF::create();
    scene->addChild(layer);

    return scene;
}

bool IntroSceneF::init()
{
	...
	this->scheduleOnce(schedule_selector(IntroSceneF::start2), 5.0f);

    return true;
}
void IntroSceneF::update(float ft)
{
	//씬 자체적으로 반복 되야하는 내용
}
void IntroSceneF::InitBackGround()
{
	auto sprite = Sprite::create("MiniGameResources/TITLE5.png");
	sprite->setPosition(Point(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y));
	this->addChild(sprite, 0);
}
void IntroSceneF::InitLabel()
{
	char MyString[100];
	WideCharToMultiByte(CP_UTF8, 0, L"그래서 주인공은 계획을 세워 복수하기위해 움직이기 시작하는데...", -1, MyString, 100, NULL, NULL);
...
void IntroSceneF::InitMenu()
{
	...
	auto storyA_item = MenuItemImage::create(
		...
		CC_CALLBACK_1(IntroSceneF::storyAspr, this));
	...
	auto skip_item = MenuItemImage::create(
		...
		CC_CALLBACK_1(IntroSceneF::skipEspr, this));
	...
	//	CC_CALLBACK_1(IntroSceneF::replayspr, this));
...
void IntroSceneF::start2(float f)
{
	
	log("start1 after delay");
	auto scene = TransitionCrossFade::create(1.5, IntroScene::createScene());
	Director::getInstance()->replaceScene(scene);
	/*Director::getInstance()->getEventDispatcher()->removeAllEventListeners();
	Director::getInstance()->replaceScene(IntroScene::createScene());*/
	
}
void IntroSceneF::storyAspr(Ref *pSender)
{

}
void IntroSceneF::skipEspr(Ref *pSender)
{
