...
Scene* MiniGameMainScene_2::createScene()
{
    auto scene = Scene::create();
    auto layer = MiniGameMainScene_2::create();
    scene->addChild(layer);

    return scene;
}

bool MiniGameMainScene_2::init()
{
	...
	// 스케쥴
	this->schedule(schedule_selector(MiniGameMainScene_2::update));
	this->schedule(schedule_selector(MiniGameMainScene_2::Score));
	this->schedule(schedule_selector(MiniGameMainScene_2::SetLabel), 1.0f);
	this->schedule(schedule_selector(MiniGameMainScene_2::SetGarbage), 0.8f);
	// 키보드 리스너
	auto keylistener = EventListenerKeyboard::create();
	keylistener->onKeyPressed = CC_CALLBACK_2(MiniGameMainScene_2::onKeyPressed, this);
	keylistener->onKeyReleased = CC_CALLBACK_2(MiniGameMainScene_2::onKeyReleased, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(keylistener, this);

    return true;
}
void MiniGameMainScene_2::update(float ft) // 씬 자체적으로 반복
{
	...
	for (int i = 0; i < 30; i++)
	{
		...
		{
			Singletons::getInstance()->GameScore += 1;
	...
	for (int i = 30; i < 60; i++)
	{
		...
		{
			Singletons::getInstance()->GameScore += 3;
	...
	for (int i = 60; i < 70; i++)
	{
		...
		{
			Singletons::getInstance()->GameScore -= 2;
...
void MiniGameMainScene_2::InitBackGround() // 뒷배경
{
	auto sprite = Sprite::create("ServiceMiniGame/volunteer.png");
	sprite->setPosition(Point(visibleSize.width / 2 + origin.x, visibleSize.height / 2 + origin.y));
	this->addChild(sprite, 0);
}
void MiniGameMainScene_2::InitSprite() // 사람
{
...
void MiniGameMainScene_2::SetGarbage(float delta) // 쓰레기
{
...
void MiniGameMainScene_2::InitLabel() // 시간, 점수 설정
{
	...
	Label* Score = Label::createWithTTF(StringUtils::format("", Singletons::getInstance()->GameScore), "MiniGameResources/CuteFont.ttf", 30);
...
void MiniGameMainScene_2::SetLabel(float delta) // 시간 변화
{
	...
	if (currClock == 0) // 시간이 다 될 경우
	{
		...
		Director::getInstance()->replaceScene(MiniGameEnd::createScene());
		Singletons::getInstance()->bongnum = 2;
	}
}
void MiniGameMainScene_2::Score(float ft) // 점수 변화
{
	Label* label = (Label*)this->getChildByTag(20);
	label->setString(StringUtils::format("%d", Singletons::getInstance()->GameScore));
}
void MiniGameMainScene_2::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event) // 방향키 처리
{
...
void MiniGameMainScene_2::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)
{
	auto spr = (Sprite*)this->getChildByTag(1);
	switch (keyCode)
	{
