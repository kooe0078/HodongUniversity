...
bool CardScene::init()
{
	...
	sprReady();
	this->scheduleOnce(schedule_selector(CardScene::sprStart), 1.1f); // GO 효과 스케줄 처리
	this->scheduleOnce(schedule_selector(CardScene::actionCard), 2.2f); // 카드 오픈 처리
	this->scheduleOnce(schedule_selector(CardScene::endCard), 10.2f); // 순서 확인 후 시작 처리

    return true;
}
bool CardScene::onTouchBegan(Touch * touch, Event * unused_event) // 터치 리스너
{
...
void CardScene::initTopMenuLabel() // 카드 수, 생명 상단 배치
{
...
void CardScene::setLabelCard() // 카드 수
{
...
void CardScene::sprStart(float delta) // 시작 로고
{
...
void CardScene::actionCard(float delta) // 카드 오픈
{
	for (int i = 0; i < 8; i++)
	{
...
void CardScene::initO() // 맞으면 O창이 나옴
{
...
void CardScene::initX() // 틀리면 X창이 나옴
{
...
void CardScene::actionOX(bool isO) // OX창 관리
{
...
void CardScene::endOX() // 종료 조건이 아닐경우
{
...
void CardScene::actionGameEnd(bool isGameClear) // 종료 조건 충족시
{
	isTouch = false;
