...
void ParkTalk::InitMenu()
{
	...
	char MyString[100];
	WideCharToMultiByte(CP_UTF8, 0, L"호동: 역시 공원은 최고야~", -1, MyString, 100, NULL, NULL);
	auto name = Label::createWithTTF(MyString, "ParkTalk/ABC.ttf", 30);
...
void ParkTalk::storyAAAAAAA(Ref *pSender)
{
	...
	auto textA_item = MenuItemImage::create(
		"ParkTalk/ok_button.png",
		"ParkTalk/ok_buttonpr.png",
		CC_CALLBACK_1(ParkTalk::storyfin, this));
	textA_item->setAnchorPoint(Point(0.5, 0.5));
	textA_item->setPosition(Point(950, 65));
...
void ParkTalk::storyBBBBBBB(Ref * pSender)
{
	...
	char MyString[100];
	WideCharToMultiByte(CP_UTF8, 0, L"킬러비 : 컹컹!\n\n(친밀도가 1 상승하였습니다.)", -1, MyString, 100, NULL, NULL);
	auto name = Label::createWithTTF(MyString, "ParkTalk/ABC.ttf", 30);
	name->setAnchorPoint(Point(0, 1));
	name->setPosition(Point(40, 150));
	name->setColor(Color3B::WHITE);
...
void ParkTalk::GoHome(Ref *pSender)
{
	...
	Director::getInstance()->replaceScene(ParkTalk::createScene());
}
